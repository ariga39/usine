...
For official `@openai/codex-sdk` 0.154.0, interim usage comes only from the matching invocation's own session artifact under the configured `CODEX_HOME`. Missing artifacts and unreported tails remain unknown; reported cumulative subtotals are never estimates or complete totals. This is bounded observation at meaningful SDK boundaries, with no polling lifecycle and no historical backfill.
