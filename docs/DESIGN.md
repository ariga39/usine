...
The guardian freezes the accepted Outcome Tree and supplies a complete bounded initial Task frontier before handing execution to the factory. The explicit handoff freezes initial proposal supply; before handoff, an empty or partial frontier is still open for guardian proposals. At an exhausted frontier, one current persisted `gaps` assessment may authorize the one separate read-only replacement-planner invocation for that Campaign/Outcome opportunity; the assessment and evidence snapshot remain its lineage. The coordinator validates its strict candidate against immutable Goal/Outcome identity, authority, prior ownership, and remaining budgets before appending it; the planner cannot publish or admit work. Dependency changes and capacity release are mechanical transitions. Evidence that invalidates the plan, a second replacement opportunity, a new Outcome, or material product behavior requires a guardian-authored revision under user authority.
...
Campaign coordination is a behavior cluster owned by the runtime server's published-goal entry path. The current production slice owns committed Goal Contract validation, immutable publication identity, durable Planned/Ready projection, deterministic admission of bounded Ready proposals through the existing leaf, release of eligible successors from accepted Task facts, reduction of accepted exact-SHA delivery into live Outcome evidence, read-only Outcome assessment at an exhausted frontier or explicit checkpoint, one bounded replacement-planner opportunity after a current gaps assessment, and terminal Campaign projection; it may reuse Task Authority's SQLite host without overloading `TaskResult`, faking a per-Task GitHub Issue, or teaching Delivery Run about planning. The Campaign plan handoff replaces the guardian-authored per-Task submit/continue loop: after supplying the complete bounded plan, the guardian is an observer and exception handler.
