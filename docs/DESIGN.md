...
The implemented `SessionRequest` is specific to implementer and reviewer Tasks. Planning remains outside Coding Session: the guardian prepares the Campaign plan before handoff and does not pass a fake Task Contract through a provider adapter. Delivery Run and Quality Gate supply caller-owned Task context; Coding Session owns the concise role-quality contract that it prepends for both roles and every adapter.
