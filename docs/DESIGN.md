...
当前 bounded product path 的 direction classification 为 `correct_before_expansion`。Eligibility 仍按 active falsifier/safety-authority defect > accepted-outcome critical path > representative real task > measured bottleneck > cleanup；capacity evidence 仍限定为一个 process 与每个 repository 一个 writer lease。#151 的 active falsifier 已关闭：network interruption 先形成 durable、auditable、same-Task `waiting`，只有一个显式 retry 才能恢复；Task event 仍只是 observation，durable TaskResult 与 Authority CAS 才决定 lifecycle。#176 只保留历史 falsification，#199 只保留 hermetic precursor。Herdr、transcript、process state 和 adapter prose 不能恢复 product completion authority；probe-before-retry 与 deterministic reconciliation 仍由 durable facts 决定。
