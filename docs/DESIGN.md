...
The implemented `SessionRequest` remains specific to implementer and reviewer Tasks. Planning remains outside Coding Session: the guardian prepares the Campaign plan before handoff and does not pass a fake Task Contract through a provider adapter. The Campaign assessor uses a separate provider-neutral request carrying the immutable Goal/Outcome and bounded evidence directly, with read-only sandbox policy and durable `assessor` attribution. Delivery Run and Quality Gate supply caller-owned Task context; Coding Session owns the concise role-quality contract that it prepends for each role and every adapter.
