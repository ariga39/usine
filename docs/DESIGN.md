...
The Campaign orchestration behavior cluster owns the policy between a published goal and the existing Task admission seam. Its current caller is the goal-publication entry path; its downstream caller is the Task delivery leaf. It hides Spec-version authority, Outcome traceability, proposal admission, readiness, dependency release, bounded Planner replenishment, Campaign budgets, and terminal reduction. No package or general queue interface is selected until the first implementation caller proves that seam.
