...
| #178 / #151 | #178 证明一个 process 内 configured finite capacity、same-ID idempotency、slot release 与 lowered-capacity restart refusal；不证明更高容量的性能或分布式 runner。[#226](https://github.com/ariga39/usine/pull/226) 已记录 implementer activation 1 的 turn-phase network interruption 与 sanitized public projection，使 #151 eligible；它尚未证明 explicit retry、reviewer retry 或其它 failure class。 |
...
当前 bounded product path 的 direction classification 为 `correct_before_expansion`。Eligibility 仍按 active falsifier/safety-authority defect > accepted-outcome critical path > representative real task > measured bottleneck > cleanup；capacity evidence 仍限定为一个 process 与每个 repository 一个 writer lease。#151 当前拥有最高 eligible outcome：network interruption 不能继续自动消耗 activation，也不能被不可逆地降为 terminal blocker；它必须先形成 explicit、auditable、same-Task retry。#176 只保留历史 falsification，#199 只保留 hermetic precursor。Herdr、transcript、process state、Task event 和 adapter prose 不能恢复 product completion authority；probe-before-retry 与 deterministic reconciliation 仍由 durable facts 决定。
