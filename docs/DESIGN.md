...
| Work supply | The guardian publishes one bounded Goal Contract and complete initial set of Outcome-traced Task Proposals. An exhausted incomplete plan requires a new guardian or user decision; the factory does not invent work. |
...
| Planning handoff | The guardian owns semantic decomposition before handoff. Implementer and reviewer are the only model roles inside the current factory loop. |
...
The current implementation proves the leaf loop, host-anchored committed Goal publication, durable Campaign Planned/Ready facts, automatic admission of eligible Ready proposals into the delivery leaf, deterministic successor release after accepted delivery, Outcome evidence reduction, explicit plan handoff, and terminal Campaign projection. Same-Repository overlap remains prohibited. Automatic semantic decomposition, a distributed scheduler or runner, automatic merge without contract authority, provider negotiation, another forge, a web dashboard, and a memory/vector database remain outside the boundary. Re-entry conditions are in [`DECISIONS.md`](DECISIONS.md).
...
- Campaign completion requires evidence for every required live Outcome, not an empty Ready list or a guardian statement. An exhausted plan while required Outcomes remain produces a durable blocker and one guardian/user decision request; it never closes the Campaign or starts ad hoc Tasks.
...
The guardian freezes the accepted Outcome Tree and supplies a complete bounded initial Task frontier before handing execution to the factory. The explicit handoff freezes proposal supply; before handoff, an empty or partial frontier is still open for guardian proposals. Dependency changes and capacity release are mechanical transitions. Evidence that invalidates the plan, an incomplete exhausted frontier, a new Outcome, or material product behavior requires a guardian-authored revision under user authority; the factory does not generate replacement work.

The coordinator runs eligible Ready proposals through the delivery leaf and releases successor dependencies only from durable accepted Task facts. Within the fixed handed-off plan, every proposal attached to one live Outcome must have owning accepted delivery before that Outcome evidence exists. It records one public Outcome evidence projection per satisfied live Outcome from the owning Campaign-associated Task facts. Passing checks, PR creation, model assertions, empty frontiers, and process state do not satisfy an Outcome. A Ready predecessor is executable work, not completed work.
...
| `waiting` | The implementer suffered a retryable turn-phase interruption, a reviewer suffered a typed interruption, or delivery ended with an unresolved external effect. The record stores the resume state, activation, and any typed reviewer failure class. | Delivery Run automatically claims one fresh reviewer attempt for an eligible reviewer interruption; implementer recovery and delivery reconciliation remain explicit retry paths; expiry, drift, or invalidity blocks. |
...
- Only an implementer failure in the `turn` phase with `failureClass: "network"` or `"transient_transport"`, the Forge boundary's typed unresolved delivery outcome, or a reviewer interruption with a typed transient class (`transient_capacity`, `transient_transport`, `network`, or `timeout`) may become `waiting`. Delivery Run claims a fresh replacement reviewer while the original Candidate, passing Check Result, review budget, and deadline remain valid; each claim is one fresh bounded attempt and the durable review-cycle budget stops further recovery. If the owning run is cancelled after reserving `reviewing`, Task Authority releases that reservation to `checked` without recording a verdict so restart can make a fresh bounded attempt. Deterministic protocol, configuration, authority, cancellation, unknown, and semantic inconclusive failures remain fail-closed; partial provider state never supplies recovery authority.
...
Campaign coordination is a behavior cluster owned by the runtime server's published-goal entry path. The current production slice owns committed Goal Contract validation, immutable publication identity, durable Planned/Ready projection, deterministic admission of bounded Ready proposals through the existing leaf, release of eligible successors from accepted Task facts, reduction of accepted exact-SHA delivery into live Outcome evidence, and terminal Campaign projection; it may reuse Task Authority's SQLite host without overloading `TaskResult`, faking a per-Task GitHub Issue, or teaching Delivery Run about planning. The Campaign plan handoff replaces the guardian-authored per-Task submit/continue loop: after supplying the complete bounded plan, the guardian is an observer and exception handler.
...
The Campaign boundary adds Goal Publication, Outcome, Task Proposal, readiness, and Campaign Evidence. It hands one admitted Ready Task to the existing leaf and consumes the resulting durable Task fact; it does not expose guardian context, provider sessions, database transactions, or Git process details across the boundary.
...
Campaign recovery extends this same fact-first rule. Startup must reconcile nonterminal Campaigns, recorded Task Proposals, active Task facts, Repository heads, and capacity, then perform the single next mechanical action for each eligible Campaign. Terminal Task events and startup use this same server-owned reconciliation path: it reduces accepted Task facts into satisfied dependencies and Outcome evidence, and admits at most one durable successor per available slot. It does not need to recover a guardian or model conversation, and an already-recorded proposal or decision request is not duplicated. The operating-system service manager may restart the server, but no launcher process, PID file, hook, or external guardian becomes durable Campaign authority.
...
The implemented `SessionRequest` is specific to implementer and reviewer Tasks. Planning remains outside Coding Session: the guardian prepares the Campaign plan before handoff and does not pass a fake Task Contract through a provider adapter.
...
Campaign evidence must reuse these Task and Role Run facts rather than copy their payloads. Each admitted Task and implementer/reviewer run carries a durable Campaign/Outcome association. Existing Goal publication and proposal records capture the plan handoff; additional guardian decision-touch facts belong at Campaign scope. The Task Authority provides a typed Campaign-scoped source list and owns raw Task decode/quarantine, batched event reads, and its bounded cursor. Campaign evidence owns aggregation and public projection. Together the owning facts must allow deterministic totals for uncached input, cached input, output, model/provider identity, elapsed time, review/repair rounds, accepted deliveries, blocked proposals, and guardian touches. An episode review may propose a Project rule, skill, checker, evaluator, or routing change, but no lesson changes future authority or policy merely because a model generated it. Promotion requires versioned evidence and the applicable repository or user authority.
