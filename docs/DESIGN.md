...
| `waiting` | The implementer suffered a retryable turn-phase interruption, a reviewer suffered a typed interruption, delivery ended with an unresolved external effect, or an explicitly enabled external review gate has mutable pending evidence. The record stores the resume state, activation, and any typed reviewer failure class or public waiting diagnostic. | Delivery Run automatically claims one fresh reviewer attempt for an eligible reviewer interruption; implementer recovery, delivery reconciliation, and external-review release remain explicit retry paths; expiry, drift, or invalidity blocks. |
