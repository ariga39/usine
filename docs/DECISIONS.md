...
| D-007 | Implementer activation 初期通过 Herdr CLI 管理 fresh Codex pane/session，reviewer 保持受控 noninteractive subprocess + structured output | Issue #18 的薄纵切观察到 Herdr-managed Luna lifecycle 能产生可观察 workspace progress；Herdr 不可用时才允许 direct `codex exec -p usine-implementer` fallback。ACP 只有在稳定协议能显著改善 session resume、streaming 或 capability negotiation 时才进入。 |
