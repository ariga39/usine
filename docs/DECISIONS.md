...
| D-006 | V0 只有 Codex runtime adapter；product implementer activation 使用 fresh `codex exec -p usine-implementer`（GPT-5.6 Luna high、default tier、no fast），不把 session 当作 durable authority；fresh semantic review 使用 Sol；Herdr 仅作为可选 operator observation UI | Issue #69 supersedes the prior product implementer transport claim: direct exec is the product runtime path。Herdr SessionStart Hook 提供 session identity，pane lifecycle 提供 observation evidence，但两者都不是 individual-turn completion authority。 |
| D-007 | Implementer direct exec 必须提交 `thread.started`、`turn.started`、`turn.completed` JSONL evidence 和 schema-valid final output；worker environment 不携带 Herdr variables。Fresh reviewer 继续通过新建 Herdr pane/agent 运行显式配置的 Sol/low/default、read-only candidate checkout 并产出 rendered transcript evidence；协调器随后使用 `ai` + `@ai-sdk/openai` 的成熟 provider 对 transcript 做有界 extraction。API key 只留在 coordinator；协调器执行严格 schema 与 exact-SHA 校验，LLM prose、provider/process success 都不能授予 authority | Herdr SessionStart Hook 可报告内部 session identity 和 transcript path，pane state 也可区分 working/idle/blocked；但 pane state 和 prompt wait 不是单个 Codex turn 完成事实。Issue #69 只 supersede implementer transport，不改变 reviewer transport。 |
