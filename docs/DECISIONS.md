...
| D-007 | Implementer activation 初期通过 Herdr CLI 管理 fresh Codex pane/session；fresh reviewer 也通过新建 Herdr pane/agent 运行显式配置的 Sol/low/default、read-only candidate checkout 和 rendered transcript evidence，再由 credential-free `codex exec --json --output-schema` 对该 transcript 做有界结构化提取；仅 implementer 在 Herdr 不可用时允许 direct `codex exec -p usine-implementer` fallback | Issue #18 的薄纵切观察到 Herdr-managed Luna lifecycle 能产生可观察 workspace progress；Issue #12/28 的 live evidence 显示 Sol noninteractive review 会耗尽预算，因此 reviewer 采用同样可观察的 Herdr lifecycle；Issue #34 的 live evidence 显示 rendered transcript 需要独立的 bounded structured extraction。ACP 只有在稳定协议能显著改善 session resume、streaming 或 capability negotiation 时才进入。 |
