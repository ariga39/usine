...
status: current
updated: 2026-08-22
issue: https://github.com/ariga39/usine/issues/178
---
...
| D-006 | Domain-facing Coding Session 保持 task-oriented `run(request) -> typed observation`；supported provider remains Codex。Static opaque profile composition 在 activation 前选择 exactly one of the official `@openai/codex-sdk` adapter or the bounded local app-server adapter；两个 adapter 都留在同一个 provider-neutral Coding Session port 后面。没有 fallback、registry、capability negotiation、automatic routing、第三个/non-Codex provider 或 app-server task authority。Herdr/transcript 不进入 production correctness path，profile 内的 model、provider、reasoning、service tier、credentials 与 optional developer instructions 不进入 durable model；developer instructions 只作为 host-private secondary composition，不能替换 frozen Task Contract 或 coordinator prompt | #192 已证明 SDK lifecycle；#195 已证明 bounded app-server lifecycle 与静态 profile composition。provider execution semantics 由 adapter 拥有；Coding Session 不承诺非-Codex compatibility。 |
...
| D-027 | Issue #187：Task Authority 持久化一个有序、Task-local、bounded、sanitized event stream；外部 sink 只接受 closed non-authoritative Coding Session/recovery observations，authoritative admission/activation/fact/terminal events 只能在对应 authority transaction 内创建。TaskResult facts 仍是唯一 lifecycle authority，事件不可用于 reconciliation。Issue #219 的 server observation 只在当前 process 内通过 bounded transient wait/subscribe 暴露 Task、Repository 或 whole-server scope；它不增加 global cursor、acknowledgement、replay、resume 或 restart recovery，也不让 SQLite own subscriptions。Coding Session 的 provider-neutral session/outcome IDs 只由 durable activation/review cycle 与事件 identity 派生，不保存 Codex thread ID、transcript、tool input/output 或 raw error。 |
| D-028 | Issue #178：Persistent local server 使用正的有限 `USINE_ACTIVE_TASK_CAPACITY`（默认 1）。Task Authority 在 admission transaction 内统计 durable nonterminal Task，并与 repository writer lease 一起原子地拒绝超限新 Task；same-ID idempotent resubmission 先返回既有 Task。server startup/readiness 先检查 durable count，若已超过 lowered capacity 则拒绝 readiness；不阻塞 Task 制造空位、不启动超界 execution、不引入 queue 或 distributed scheduler | Capacity policy 的 owner 是 persistent-server admission/lifecycle cluster，durable Task/lease facts 仍由 Task Authority 保存。#178 的 hermetic public evidence 覆盖 multi-repository admission、same-ID resubmission、slot release 和 lowered-capacity restart refusal；这不扩大为 dynamic resizing、fairness、distributed runner 或 Issue #151 retry recovery。 |

当前 bounded serial product path 的 classification 为 `correct_before_expansion`。#217 记录 fund-manager Issue #52 / PR #53 的 one-contract exact-head production merge evidence；#176 保留为 historical falsification，#199 保留为 hermetic precursor。#178 的 bounded capacity evidence 已纳入当前 product path，并受 D-028 的 process-local scope 限定。#151 已实现并由 Delivery Run、Task Authority 与 loopback CLI/server seam 共同拥有：只有 implementer 的 turn-phase `network` interruption 在 activation/deadline 仍可用时持久化为 `waiting`；Task get/list/history/watch 暴露 sanitized、retryable projection，restart 与 same-ID submit 不启动它；一次 CAS-protected explicit retry 恢复同一 Task 的原始 contract/deadline/authority 并消耗下一个 activation。其它 provider failure class、reviewer interruption、product blocker、exhausted budget/deadline 仍 terminal；不增加 table、queue、scheduler 或 timer。#187、#188、#189 是彼此独立的 outcomes；#188 resource CLI 已完成，不构成永久 architecture 顺序；#195 的 bounded app-server outcome 也不授权 fallback、registry、negotiation、automatic routing 或新的 provider surface。
