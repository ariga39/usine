...
| D-006 | V0 的 provider-neutral Coding Session port 当前只有一个 production adapter：官方 `@openai/codex-sdk`。Herdr/transcript route 从 production correctness path 删除；不预建 provider registry或第二 adapter。Repository 只选择 named Codex profile；profile 内的 model、provider、reasoning、service tier 与 credentials 不进入 Usine durable model。Usine 独立保留 fresh worker、role sandbox、isolated workspace 与 controlled environment policy | SDK 已封装 local Codex thread start/resume、structured events/output、sandbox、cwd、config 与 cancellation，能删除 CLI/JSONL/transcript glue而不重建 runtime。Coding Session 通过官方 CLI profile 选择机制应用 profile，并在 SDK 不暴露 model/provider evidence 时保持 history evidence 为空。Gas City 的 session/runtime 分离和幂等 lifecycle 只作为 donor；未来 runtime 由实证触发后在同一 port替换，Herdr 最多作为 optional observer/host re-enter，永不授予 completion authority。 |
...
| D-011 | GitHub 是当前 forge/delivery surface；每个 Repository 选择 opaque `forgeProfile`，host 按该 profile 解析 Octokit + GitHub App 短期 installation token；不保留 global credential selection 或 fallback | 现有 private repos 已安装 App；不同 Repository 必须能使用不同 installation capability，且凭据不能进入 Task Contract、durable facts 或 worker。 |
...
| D-014 | GitHub Issue → branch/worktree → PR 是开发 Usine 自身的唯一任务流；第一处 checkable/green state 立即以小 commit push 并开 draft PR。Issue #76 后只开一个 full-refactor PR，用小提交逐 cluster 替换并删除旧入口；这是一次有界结构 reset，不恢复旧 backlog | Git 提供恢复、可见性、所有权和 integration，而不是进展代理。该一次性例外避免过渡 seam 被多个 PR 固化，同时仍让每个 checkpoint 可见、可回退。 |
...
| D-025 | Persistent local server 是当前 coordinator host；它用 Effect Scope、FiberMap、Clock/Duration 和 AbortSignal bridge 持有 task fiber、资源释放、deadline-aware waiting 与 graceful cancellation。CLI 只保留 `server`、`submit`、`status`、`follow`；one-shot `run` 和 `USINE_STOP_AFTER` lifecycle 已删除。server restart 从 SQLite 重入 nonterminal Task。此决定在真实 caller 触发后扩展 D-024，supersedes Effect 只限 durable decode boundary 的初始阶段；不改变 D-024 保留 Promise-based module ports 的 incremental/library-first 约束或六个领域 package ownership | CLI lifetime 不再拥有 admitted execution，operator 可断开并重连；Effect 只接管 composition-root lifecycle，Codex SDK、Execa、Octokit、Node HTTP 与 SQLite 仍是窄 adapter。PR #82/#83 保留真实 Codex delivery/restart evidence；Issue #153 的 server-process milestone 使用 stubbed Codex SDK adapter，同时验证同一 durable Task 的 normal delivery 与 SIGKILL/restart recovery，未引入第二 scheduler、database 或 agent runtime。 |
