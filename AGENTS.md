...
The primary orchestrator owns direction, task selection, repository-agent policy, integration, review, delivery, and continuation. Product implementation uses a fresh GPT-5.6 Luna worker with high reasoning, the default service tier, and never fast. Exact-SHA review uses a fresh Sol context with a read-only candidate.
