...
- Every implementation Issue creates a new Herdr pane and a fresh Codex agent with `-p usine-implementer` (GPT-5.6 Luna high, `service_tier = "default"`, no fast); Herdr is the launcher and operator observation UI for this repository workflow, and the agent/pane is never reused across Issues. Its SessionStart Hook reports Codex session identity, while its pane lifecycle remains observation evidence; neither pane state nor prompt wait is authority for an individual Codex turn. The Usine product runtime implementer path is direct `codex exec -p usine-implementer`. Fresh semantic review uses Sol, and bounded read-only research may use Terra.
